...
const sql = await fs.readFile('postgres/003-behavior-events.sql', 'utf8');
...
async function verify(db) {
  ...
  const server = {
    database: () => db,
    limited: async () => {},
    admin: async () => ({ role, userId: 'fixture' }),
    HttpError,
    fail: (e) =>
  ...
  await db.prepare("INSERT INTO contents(id,slug,kind,data) VALUES(?,?,?,?)").bind('article-one','sample','articles',JSON.stringify({titleZh:'样例文章'})).run();
  await ingest(req({id:'r'.repeat(20),event:'article_read',target:'article-one'}),'website');
  assert.equal((await (await get(new Request(url))).json()).articles[0].title,'样例文章');
  role = 'staff';
