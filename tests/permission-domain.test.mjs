...
import {
  legacyOrderUpdate,
  legacyOrderSql,
} from "../lib/permission-domain.mjs";
import { DatabaseSync } from "node:sqlite";
test("legacy grant update changes only selected email and detects stale revisions", () => {
  const old = {
    methods: [{ id: "bank" }],
    grants: { "a@example.test": ["view"], "b@example.test": ["finance"] },
  };
  const next = legacyOrderUpdate(old, {
...
test("legacy grant compare-and-swap and audit are atomic; stale writes do not emit success records", () => {
  ...
  try {
    db.exec(
      "CREATE TABLE settings(id TEXT PRIMARY KEY,data TEXT); CREATE TABLE audit_logs(id TEXT PRIMARY KEY,actor TEXT,action TEXT,target TEXT,created_at TEXT)",
    );
    const before = JSON.stringify({
      enabled: true,
      grants: { "a@example.test": ["view"] },
    });
    ...
    const save = (old, marker, auditId) => {
      ...
      try {
        const result = db
          .prepare(legacyOrderSql.update)
          .run(
            JSON.stringify({
              enabled: true,
              grants: { "a@example.test": [] },
              permissionRevision: marker,
            }),
            old,
          );
        db.prepare(legacyOrderSql.audit).run(
          auditId,
          "owner@example.test",
          "snapshot",
    ...
    assert.equal(
      db.prepare("SELECT count(*) AS total FROM audit_logs").get().total,
      1,
    );
...
test("server permission map distinguishes baseline, visitor and configuration operations", () => {
