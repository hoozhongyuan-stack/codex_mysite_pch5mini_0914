...
产品目标是支持 Docker 的私有化部署，但当前检查未发现完整 Dockerfile / Compose 交付配置，需求也明确生产尚未完成。以下仍需实施或确认：

1. 网站 Worker/D1/R2 能力在目标私有环境中的保留或替换方案。
2. Django 生产 WSGI/ASGI 服务及反向代理、HTTPS；开发 runserver 不能直接作为生产方案。
3. 数据库备份恢复、私有存储挂载、视频磁盘配额与文件生命周期。
4. 转码、通知、订单和积分任务的常驻托管、失败告警与恢复。
5. 正式域名、OAuth 回调、小程序合法域名、SMTP 及支付接入。
6. 跨数据库故障恢复、并发库存压力、终端兼容与生产验收。

Nginx、Gunicorn、PostgreSQL、MySQL、Redis、Celery、Kubernetes 等都不能在当前文档中标为已采用；具体生产选型需在后续部署方案中确定。
