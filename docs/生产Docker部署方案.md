...
Web 镜像在构建及运行时将 `NEXT_PUBLIC_DEPLOY_ENV` 标记为 `Production`，供后台版本信息展示；更改已有浏览器产物的环境标记需要重新构建镜像。小程序 CI 子进程使用 `MINI_CI_OUTPUT_DIR/work` 作为私有工作目录，预览码保留在 `MINI_CI_OUTPUT_DIR`。该目录必须位于只供 Web 服务使用的可写持久卷，不能将 `/app` 改成全局可写或让 Web 服务以 root 运行。
