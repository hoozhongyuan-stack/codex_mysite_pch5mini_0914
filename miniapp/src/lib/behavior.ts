...
export function behavior(event:string,target?:string,shareRef=''){try{
 ...
 void Taro.request({url:origin+'/api/mini/behavior',method:'POST',data:{id:randomId(),visitorId,sessionId:session.id,event,path,target,shareRef,consent:true,channel:'mini'},header:{'Content-Type':'application/json'},timeout:5000}).catch(()=>{});
}catch{/* Optional telemetry never blocks the app. */}}
export function pageBehavior(target=''){const route=Taro.getCurrentInstance().router;const path=route?.path||'';target=/^\/?pages\/(detail|videos|salons)\/index$/.test(path)?route?.params?.id||target:target;const shareRef=String(route?.params?.share||'');const key=path+':'+target+':'+shareRef;try{if(Taro.getStorageSync(choiceKey)!=='accepted')return;if(key!==lastPage||Date.now()-lastAt>1800000){behavior('page_view',target||undefined,shareRef);lastPage=key;lastAt=Date.now();}}catch{}}
