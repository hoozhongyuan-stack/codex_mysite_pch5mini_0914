...
export default function MemberOverview({user,points,avatarUrl,onOpen,onLogin,onShop,loading=false}:any){
 return <View className="member-overview"><View className="member-profile">{avatarUrl?<Image className="member-avatar member-avatar-image" src={avatarUrl} mode="aspectFill"/>:<View className="member-avatar">{user?memberName(user).slice(0,1):'客'}</View>}<View><Text className="member-caption">{user?'欢迎回来':'随心浏览，发现好物'}</Text><Text className="member-name">{memberName(user)}</Text>{!user&&!loading&&<ActionButton size="mini" onClick={onLogin}>登录 / 注册</ActionButton>}</View>{user&&<ActionText className="member-setting" onClick={()=>onOpen('settings')}>设置</ActionText>}</View>
