...
export function normalizeEvent(input, channel) {
  ...
  const shareRef = privatePath ? '' : input.shareRef || '';
  if (
    ...
    throw Error('内容标识无效');
  if (typeof shareRef !== 'string' || (shareRef && !/^[a-z0-9]{24}$/.test(shareRef))) throw Error('分享标识无效');
  return {
    id: input.id,
...
export function summarizeBehavior(rows) {
  ...
  return {
    ...
    shares: rows.filter((r) => r.event === 'share').length,
    shareReturns: rows.filter((r) => r.event === 'page_view' && r.share_ref).length,
