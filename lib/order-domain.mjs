...
export function orderConfig(input) {
  ...
  const mini = miniPayments(input.miniPayments);
  const afterSaleDays = input.afterSaleDays;
  if (enabled || mini.offline) integer(afterSaleDays, 0, 3650);
  ...
  if ((enabled || mini.offline) && !methods.some((m) => m.enabled))
    throw Error('启用交易前至少配置一种收款方式');
  return {
    enabled,miniPayments:mini,
