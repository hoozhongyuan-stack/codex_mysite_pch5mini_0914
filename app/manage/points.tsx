...
export default function PointsManager() {
  ...
  return <section className="points-workspace"><header className="points-heading"><div><h1>积分管理</h1><p>管理访客积分、查看收支记录与配置奖励规则。</p></div></header>
    <nav className="points-tabs" aria-label="积分功能">{[['accounts','积分账户'],['ledger','积分明细'],['rules','奖励规则']].map(([key,label]) => <button type="button" key={key} aria-pressed={tab===key} onClick={() => setTab(key)}>{label}</button>)}</nav>
    ...
    <div className="table-wrap"><table><thead><tr><th>奖励规则</th><th>启用</th><th>奖励积分</th><th>操作</th></tr></thead><tbody>
