...
export default function MiniContentPicker({target, value, onSelect, onClose, microPages=[]}: any) {
  const kind = ({product:'products',article:'articles',form:'forms',video:'videos',event:'events',microPage:'microPages'} as Record<string,string>)[target];
  ...
  useEffect(()=>{
    if(target==='microPage'){const filtered=(microPages||[]).filter((row:any)=>row.status==='published'&&(!query||row.title?.includes(query)));setRows(filtered.map((row:any)=>({...row,titleZh:row.title})));setPages(1);setLoading(false);setError('');return;}
  ...
  },[kind,target,query,page,retry,microPages]);
  return <Dialog guardChanges={false} open onOpenChange={open=>!open&&onClose()}><DialogContent size="lg" className="mini-content-dialog"><DialogHeader><DialogTitle>选择关联内容</DialogTitle><DialogDescription>{target==='microPage'?'仅展示已发布的微页面。':['video','event'].includes(target)?'视频与沙龙会沿用已发布的公开内容。':'仅展示已发布且启用小程序渠道的内容。'}</DialogDescription></DialogHeader>
