...
export async function GET(request: Request) {
  try {
    ...
    const rows = (
      await db
        .prepare(
          "SELECT channel,visitor_hash,session_hash,event,path,target,share_ref,created_at FROM behavior_events WHERE created_at>=? AND created_at<? AND (?='all' OR channel=?) ORDER BY created_at LIMIT 20001",
        )
