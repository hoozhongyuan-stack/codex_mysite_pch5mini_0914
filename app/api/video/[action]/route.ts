...
export async function GET(request: Request, { params }: any) {
  try {
    ...
    if (action === 'admin-source-preview') {
      ...
      const r = await internal('video-source-preview', JSON.stringify({id: data.id}), {
        'Content-Type': 'application/json', 'X-Staff-Session': staff(request),
        ...(request.headers.get('range') ? {Range: request.headers.get('range')!} : {}),
      });
      const headers = new Headers({'Cache-Control':'private, no-store','X-Content-Type-Options':'nosniff'});
      for (const name of ['content-type','content-length','content-range','accept-ranges']) {
        const value = r.headers.get(name); if (value) headers.set(name,value);
      }
    ...
    if (action.startsWith('admin-')) {
      await admin('saveSettings');
      const folders = action === 'admin-library' ? await database().prepare('SELECT id FROM asset_folders').all() : null;
      return Response.json(
        await identity('admin-video-' + action.slice(6), {
          ...data,
          ...(folders ? { _folders: folders.results.map((f: any) => f.id) } : {}),
    ...
    if (
      ...
      if (['admin-save-series', 'admin-save-episode'].includes(action)) {
        for (const key of ['bodyZh', 'bodyEn']) {
          try { data[key] = videoBody(data[key]); } catch (e) { throw new HttpError(400, (e as Error).message); }
        }
        data.id = data.id || crypto.randomUUID();
        for (const assetId of new Set([...richAssets(data.bodyZh), ...richAssets(data.bodyEn)])) {
          if (!(await database().prepare('SELECT id FROM assets WHERE id=?').bind(assetId).first())) throw new HttpError(400, '详情素材不存在');
          await database().prepare('INSERT OR IGNORE INTO marketing_assets(id,event_id,asset_id) VALUES(?,?,?)').bind('video:'+data.id+':'+assetId, 'video:'+data.id, assetId).run();
        }
      }
      if (
