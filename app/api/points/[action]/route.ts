...
export async function GET(request: Request, { params }: any) {
  try {
    ...
    if (isAdmin) await admin("readPoints");
    const data = isAdmin
      ? {
    ...
    return Response.json(
      await identity(
        isAdmin ? "admin-points-" + action : "points-" + action,
        data,
      ),
...
export async function POST(request: Request, { params }: any) {
  try {
    ...
    if (action === "interact") {
      ...
      if (["article", "product"].includes(input.kind)) {
        ...
        const row = await database()
          .prepare(
            "SELECT data,slug FROM contents WHERE id=? AND kind=? AND status='published' AND COALESCE(json_extract(data,'$.channels.website'),1)=1",
          )
          .bind(String(input.id), kind)
          .first<any>();
        if (!row) throw new HttpError(404, "内容尚未发布");
        title = JSON.parse(row.data).titleZh || "";
        path = "/zh/" + kind + "/" + row.slug;
      } else if (!["salon", "video"].includes(input.kind))
        throw new HttpError(400, "互动类型无效");
      return Response.json(
        await identity("points-interact", {
          kind: input.kind,
          id: input.id,
          action: input.action,
          active: input.active,
    ...
    const user = await admin("managePoints");
    await limited("points-admin:" + user.userId, 100);
    if (action === "retry-orders") {
      const rows = (
        await database()
          .prepare(
            "SELECT id FROM orders WHERE json_extract(data,'$.pointsPending')=1 ORDER BY updated_at LIMIT 100",
          )
          .all<any>()
      ).results;
      let failed = 0;
    ...
    if (action === "retry-forms") {
      const rows = (
        await database()
          .prepare(
            "SELECT id,data FROM submissions WHERE json_extract(data,'$._points.status') IN ('pending','failed') ORDER BY CASE json_extract(data,'$._points.status') WHEN 'pending' THEN 0 ELSE 1 END,created_at,id LIMIT 100",
          )
          .all<any>()
      ).results;
      let failed = 0;
      for (const row of rows) {
        try {
          ...
          await database()
            .prepare(
              "UPDATE submissions SET data=json_set(data,'$._points.status','done') WHERE id=?",
            )
            .bind(row.id)
            .run();
        } catch {
          ...
          await database()
            .prepare(
              "UPDATE submissions SET data=json_set(data,'$._points.status','failed') WHERE id=?",
            )
            .bind(row.id)
            .run();
        }
    ...
    if (!["save-rule", "adjust"].includes(action))
      throw new HttpError(404, "未知操作");
    await limited("points-admin:" + user.userId, 100);
