...
export async function GET(request: Request) {
  try {
    ...
    if (url.searchParams.get('admin') === '1') {
      ...
      if (user.role !== 'owner') throw new HttpError(403, '仅站点管理员可配置');
      const forms = (await database().prepare("SELECT id,status,json_extract(data,'$.channels.mini') AS mini,json_extract(data,'$.channels.website') AS website FROM contents WHERE kind='forms'").all<any>()).results;
      return Response.json({ ...await channelState(), formAvailability: forms }, {
    ...
    return Response.json(
      {
        ...
        capabilities:
            ...
            ? ['home', 'products', 'articles', 'points','account','cart','videos','events']
            : undefined,
